...
class _DashboardState extends State<Dashboard> {
  ...
  void didChangeDependencies() {
    ...
    // Refresh sales and purchases amounts when dependencies change (e.g., returning to dashboard)
    final inventoryProvider = Provider.of<Inventoryprovider>(
      context,
      listen: false,
    );
  ...
  void _refreshDashboardWithDateRange() {
    try {
                          ...
                          children: [
                              ...
                              onSuggestionSelected: (suggestion) {
                                setModalState(() {
                                  ...
                                  final quantity =
                                      ...
                                      0;
                                  final unitPrice =
                                      int.tryParse(unitPriceController.text) ??
                                      0;
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                ...
                                if (_debtSelected) {
                                  if (_debtCustomerNameController
                                          .text
                                          .isEmpty ||
                                      _debtCustomerPhoneController
                                          .text
                                ...
                                // Validate payment sum equals total amount unless debt is selected
                                final int totalAmount = chartItems.fold(
                                  0,
                                  (sum, item) =>
                                      sum + (item['total_price'] as int),
                                );
                                ...
                                if (!_debtSelected && paidSum != totalAmount) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        'Payment total (RWF $paidSum) must equal total amount (RWF $totalAmount)',
                                      ),
                                      backgroundColor: Colors.red,
                                    ),
                                  );
                                ...
                                if (_debtSelected &&
                                    paidSum + debtVal != totalAmount) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        'Paid + Debt (RWF ${paidSum + debtVal}) must equal total amount (RWF $totalAmount)',
                                      ),
                                      backgroundColor: Colors.red,
                                    ),
                                  );
                                ...
                                List<SaleItem> saleItems = chartItems.map((
                                  item,
                                ) {
                                  return SaleItem(
                                    name: item['name'],
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                if (formKey.currentState!.validate()) {
                                  ...
                                  if (await Inventoryprovider()
                                      .addPurchaseRecord(item)) {
                                    formKey.currentState?.reset();
                                    _searchController.clear();
                                    _descriptionController.clear();
                                    _quantityController.clear();
